DELIMITER $$
CREATE PROCEDURE `get_dashboard_by_customer_exchange_and_currency`(IN dashboard_base_table_name VARCHAR(45))
BEGIN
...
AS
(
SELECT customer_name, exchange, currency, SUM(balance_before) AS balance_before, SUM(balance_current) AS balance_current, 
...
GROUP BY customer_name, exchange, currency
);

INSERT INTO overall
SELECT customer_name, exchange, 'TOKEN' AS currency, SUM(balance_before) AS balance_before, SUM(balance_current) AS balance_current, 
...
GROUP BY customer_name, exchange;

DROP TEMPORARY TABLE IF EXISTS tmp;
SELECT * FROM overall ORDER BY customer_name, exchange, currency;
